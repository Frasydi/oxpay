...
const Login = () => {
  ...
  const handleOXPayLogin = async () => {
    setLoading(true);
    try {
      const response = await authService.loginWithOXPay();
      console.log('OXPay login successful:', response);
      alert('OXPay login successful!');
    } catch (error: any) {
      setError(error.message || 'OXPay login failed');
    } finally {
      setLoading(false);
    }
  };
